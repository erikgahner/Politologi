...
polls_use %>%
  ...
  drop_na(support) |> 
  filter(!party %in% c("party_e", "party_p", "party_g")) %>%
  ...
  labs(y = "Stemmer (%)",
       x = NULL) +
  scale_colour_manual(labels = c("Socialdemokraterne", "Alternativet", "Danmarksdemokraterne", "Radikale Venstre", "Konservative", "Nye Borgerlige", "SF", 
                                 "Liberal Alliance", "Kristendemokraterne", "Moderaterne", "Dansk Folkeparti", "Enhedslisten", "Frie Grønne", "Venstre"), 
                      values = c("#E3515D", "#AEFEAF", "#b23835", "#EB4295", "#429969", "#05454F", "#9C1D2A", 
                                 "#EE9A5F", "#F4CE97", "#5bc3f5", "#3D6F8D", "#914A4F", "green", "#459BC8"),
